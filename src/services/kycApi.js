class KYCApiService {
  ...
  // 🧾 OCR
  async processOCRDocument(image, uuid) {
    const formData = new FormData();
  ...
  // 🔍 MRZ (Local Engine)
  async processMRZDocument(image, uuid) {
    const formData = new FormData();
  ...
  async processBarcodeDocument(image, uuid) {
    const formData = new FormData();
    const filename = `mrz-${uuid}.jpg`;
    ...
    try {
      ...
      console.error('[processBarcodeDocument] API call failed:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error occurred',
        message: 'Failed to process Barcode request',
      };
    }
  }
